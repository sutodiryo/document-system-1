...
Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'check_block']], function () {
    ...
    Route::get('lock', [DocumentController::class, 'lock'])->name('documents.lock');
    Route::get('unlock', [DocumentController::class, 'unlock'])->name('documents.unlock');

    Route::group(['prefix' => '/document-mail', 'as' => 'documents.mail.'], function () {
        Route::post('share/{id}', [EmailController::class, 'SendShareDocumentEmail'])->name('share');
        Route::post('approval/{id}', [EmailController::class, 'SendApprovalDocumentEmail'])->name('approval');
    });

    Route::post('store-folder', [DocumentController::class, 'storeFolder'])->name('documents.store-folder');
